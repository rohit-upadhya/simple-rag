...
class Prompter:
    def __init__(
        self, context_text: Text, query_text: Text, prompt_template: Dict[Any, Any]
    ):
    ...
    def build_chat_prompt(
        ...
        if "system_prompt" in self.prompt_template:
            content = self.prompt_template.get("system_prompt", "").format(
                context_text=self.context_text
            )
            final_prompt.append({"role": "system", "content": content})

        if self.query_text is not None:
            final_prompt.append({"role": "user", "content": self.query_text})
        else:
